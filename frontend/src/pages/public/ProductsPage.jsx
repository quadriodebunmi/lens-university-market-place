...
<p style={{ color:'rgba(255,255,255,0.6)', fontSize:'0.95rem' }}>{total !== null ? `Browse ${total} product${total!==1?'s':''} from our sellers` : 'Loading…'}</p>
</div>
</div>
...
{loadingInit && <div className="grid-4 product-grid">{Array.from({length:8}).map((_,i)=><div key={i} className="card skeleton-card"/>)}</div>}
